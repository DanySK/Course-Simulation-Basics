...
\begin{frame}{Simulation and reproducibility}
    ...
    \begin{itemize}
        ...
        \item \textit{``Strict'' reproducibility}: used in computer science, is the ability to obtain the exact same output on the exact same input data.
    \end{itemize}
...
Problem: we have a container with a precise number of molecules that may react with each other. We want to forecast the evolution of the system in the future.
...
\begin{block}{Details}
 \begin{itemize}
  \item $P(\mu,\tau) = a_\mu e^{-\tau\sum_j{a_j}}$: the probability that reaction $\mu$ occurs at time $\tau$ is its speed times the probability distribution. Being this a Poisson process, the probability distribution is a negative exponential function, whose exponent is the sum of the speeds of all the reactions in the system.
 \end{itemize}
\end{block}
\end{frame}
...
\begin{frame}{Next reaction: random reuse}
 \begin{block}{Random generation}
  \begin{enumerate}
   ...
   \item In a purely chemical simulator, it is the heaviest task \cite{GibsonJPC1999}
 ...
 \begin{block}{Random reuse}
  \begin{itemize}
   ...
   \begin{itemize}
    ...
    \item This is possible due to the exponential distribution being \emph{memoryless}
...
\begin{frame}{From chemistry to pervasive computing}
 ...
 \end{block}
 \begin{block}{Idea}
  Instead of using classic ABSs and optimizing at the simulation level, can we take a kinetic Monte Carlo and extend it until it supports all the abstractions we need?
 \end{block}
\end{frame}
...
\begin{frame}[fragile]{Personalised time distribution}
	...
	\begin{itemize}
		...
		\item The Alchemist loader automatically assigns values to arguments of type \texttt{Environment}, \texttt{Incarnation}, \texttt{RandomGenerator}, \texttt{Node}, \texttt{Reaction}, \texttt{TimeDistribution} depending on the context, letting the user specify only the parts strictly required.
	\end{itemize}
\end{frame}
